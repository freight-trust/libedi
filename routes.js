...
var JsonSchemaGenerator = /** @class */ (function () {
  function JsonSchemaGenerator(
    ...
    this.tc = tc;
    this.userValidationKeywords = args.validationKeywords.reduce(function (
      acc,
      word
    ) {
      var _a;
   ...
   */
  JsonSchemaGenerator.prototype.parseCommentsIntoDefinition = function (
    symbol,
    definition,
    otherAnnotations
  ) {
    ...
    jsdocs.forEach(function (doc) {
      ...
      if (subDefinitions[name]) {
        ...
        if (match) {
          var k = match[1];
          var v = match[2];
          definition[name] = __assign(
            __assign({}, definition[name]),
            ((_a = {}), (_a[k] = v ? parseValue(v) : true), _a)
          );
          return;
        }
      }
      ...
      if (name.includes(".")) {
        ...
        if (parts.length === 2 && subDefinitions[parts[0]]) {
          definition[parts[0]] = __assign(
            __assign({}, definition[parts[0]]),
            ((_b = {}), (_b[parts[1]] = text ? parseValue(text) : true), _b)
          );
        }
      }
  ...
  JsonSchemaGenerator.prototype.getDefinitionForRootType = function (
    propertyType,
    reffedType,
    definition,
    defaultNumberType
    ...
    if (tupleType) {
      ...
      var fixedTypes = elemTypes.map(function (elType) {
        return _this.getTypeDefinition(elType);
      });
      ...
      var propertyTypeString = this.tc.typeToString(
        propertyType,
        undefined,
        ts.TypeFormatFlags.UseFullyQualifiedType
      );
      var flags = propertyType.flags;
      var arrayType = this.tc.getIndexTypeOfType(
        propertyType,
        ts.IndexKind.Number
      );
      if (flags & ts.TypeFlags.String) {
        ...
        var isInteger =
          ...
          (reffedType && reffedType.getName() === "integer") ||
  ...
  JsonSchemaGenerator.prototype.getDefinitionForProperty = function (
    prop,
    node
  ) {
    ...
    var definition = this.getTypeDefinition(
      propertyType,
      undefined,
      undefined,
      prop,
  ...
  JsonSchemaGenerator.prototype.getEnumDefinition = function (
    clazzType,
    definition
  ) {
    var _this = this;
    var node = clazzType.getSymbol().getDeclarations()[0];
    var fullName = this.tc.typeToString(
      clazzType,
      undefined,
      ts.TypeFormatFlags.UseFullyQualifiedType
    );
  ...
  JsonSchemaGenerator.prototype.getUnionDefinition = function (
    unionType,
    prop,
    unionModifier,
    definition
  ...
  JsonSchemaGenerator.prototype.getIntersectionDefinition = function (
    intersectionType,
    definition
  ) {
    var simpleTypes = [];
  ...
  JsonSchemaGenerator.prototype.getClassDefinition = function (
    clazzType,
    definition
  ) {
    var _this = this;
    ...
    if (
      ...
      if (clazz.members) {
        var indexSignatures =
          clazz.members == null
            ? []
            : clazz.members.filter(function (x) {
                return x.kind === ts.SyntaxKind.IndexSignature;
              });
  ...
  JsonSchemaGenerator.prototype.getTypeDefinition = function (
    typ,
    asRef,
    unionModifier,
    prop,
    ...
    if (asTypeAliasRef) {
      ...
      if (this.args.uniqueNames) {
        ...
        var relativePath = path.relative(process.cwd(), sourceFile.fileName);
        fullTypeName =
          this.getTypeName(typ) +
          "." +
          generateHashOfNode(getCanonicalDeclaration(sym), relativePath);
      } else if (
        reffedType &&
    ...
    if (!asRef || !this.reffedDefinitions[fullTypeName]) {
      ...
      var node =
        symbol && symbol.getDeclarations() !== undefined
  ...
  JsonSchemaGenerator.prototype.setSchemaOverride = function (
    symbolName,
    schema
  ) {
  ...
  JsonSchemaGenerator.prototype.getSchemaForSymbols = function (
    symbolNames,
    includeReffedDefinitions
  ) {
    if (includeReffedDefinitions === void 0) {
    ...
    for (
      var _i = 0, symbolNames_1 = symbolNames;
      _i < symbolNames_1.length;
      _i++
    ) {
    ...
    if (
      ...
      root.definitions = __assign(
        __assign({}, root.definitions),
        this.reffedDefinitions
      );
    }
    return root;
...
function getProgramFromFiles(files, jsonCompilerOptions, basePath) {
  ...
  var compilerOptions = ts.convertCompilerOptionsFromJson(
    jsonCompilerOptions,
    basePath
  ).options;
  var options = {
    ...
    if (matchingSymbols.length === 1) {
      ...
      throw new Error(
        matchingSymbols.length +
          ' definitions found for requested type "' +
          fullTypeName +
          '".'
      );
    }
...
function programFromConfig(configFileName, onlyIncludeFiles) {
  ...
  var result = ts.parseConfigFileTextToJson(
    configFileName,
    ts.sys.readFile(configFileName)
  );
  var configObject = result.config;
  var configParseResult = ts.parseJsonConfigFileContent(
    configObject,
    ts.sys,
...
function exec(filePattern, fullTypeName, args) {
  ...
  if (REGEX_TSCONFIG_NAME.test(path.basename(filePattern))) {
    if (args.include && args.include.length > 0) {
      var globs = args.include.map(function (f) {
        return glob.sync(f);
      });
      onlyIncludeFiles = (_a = []).concat
        .apply(_a, globs)
